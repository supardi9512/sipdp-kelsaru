...
class KettidakmampuModel {
    ...
    public function getAll()
    {
        if($_SESSION['level'] == 'rw') {
            ...
            $this->db->query('SELECT '.$this->table.'.*, '.$this->table2.'.nama_penduduk,  '.$this->table2.'.tempat_lahir,  '.$this->table2.'.tgl_lahir,  
                ...
                INNER JOIN '.$this->table3.' ON '.$this->table2.'.id_rw = '.$this->table3.'.id_rw 
                INNER JOIN '.$this->table4.' ON '.$this->table2.'.id_rt = '.$this->table4.'.id_rt
                WHERE '.$this->table4.'.id_rt = :id_rt ORDER BY '.$this->table.'.no_tidak_mampu DESC');
            ...
            $this->db->query('SELECT '.$this->table.'.*, '.$this->table2.'.nama_penduduk,  '.$this->table2.'.tempat_lahir,  '.$this->table2.'.tgl_lahir,  
                ...
                INNER JOIN '.$this->table3.' ON '.$this->table2.'.id_rw = '.$this->table3.'.id_rw 
                INNER JOIN '.$this->table4.' ON '.$this->table2.'.id_rt = '.$this->table4.'.id_rt
                WHERE '.$this->table2.'.nik = :nik ORDER BY '.$this->table.'.no_tidak_mampu DESC');
    ...
    public function getByNo($no)
    {
        $this->db->query('SELECT '.$this->table.'.*, '.$this->table2.'.nama_penduduk,  '.$this->table2.'.tempat_lahir,  '.$this->table2.'.tgl_lahir,  
            ...
            INNER JOIN '.$this->table3.' ON '.$this->table2.'.id_rw = '.$this->table3.'.id_rw 
            INNER JOIN '.$this->table4.' ON '.$this->table2.'.id_rt = '.$this->table4.'.id_rt
            WHERE '.$this->table.'.no_tidak_mampu = :no_tidak_mampu');
    ...
    {
        $this->db->query('SELECT MAX(no_tidak_mampu) AS no_tidak_mampu_terbesar FROM '.$this->table);
        return $this->db->single();
    }

    public function create($data)
    {
        $query = "INSERT INTO ".$this->table." VALUES (:no_tidak_mampu, :nik, :validasi_rt, :validasi_rw)";

        $this->db->query($query);
        $this->db->bind('no_tidak_mampu', $data['no_tidak_mampu']);
    ...
    public function updateValidasi($no, $validasi)
    {
        if($_SESSION['level'] == 'rw') {
            $query = "UPDATE ".$this->table." SET validasi_rw = :validasi_rw
                    WHERE no_tidak_mampu = :no_tidak_mampu";
